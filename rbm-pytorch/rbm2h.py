...
class RBM(nn.Module):
    ...
    def __init__(self, n_vis=784, n_hid=128, n_hid2=100, k=1):
        """Create a RBM."""
        super(RBM, self).__init__()
        self.v = nn.Parameter(torch.randn(1, n_vis))
        self.h = nn.Parameter(torch.randn(1, n_hid))
        self.h2 = nn.Parameter(torch.randn(1, n_hid2))
        self.W = nn.Parameter(torch.randn(n_hid, n_vis))
        self.W2 = nn.Parameter(torch.randn(n_hid2, n_hid))
        self.k = k
    ...
    def visible_to_hidden(self, v):
        ...
        p2 = torch.sigmoid(F.linear(p, self.W2, self.h2))
        return p2.bernoulli()

    def hidden_to_visible(self, h, h2):
        ...
        """
        p = torch.sigmoid(F.linear(h2, self.W2.t(), h))
        p2 = torch.sigmoid(F.linear(p, self.W.t(), v))
        return p2.bernoulli()
